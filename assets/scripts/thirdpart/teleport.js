(function (global, factory) {
	...
	function getProto(name) {
		if (name) {
			...
			if (!proto) {
				var arr = name.split('.');
				var ns = arr[0];
				temp = protoRootMap[ns].build(arr[1]);
				if (temp) {
	...
	function addProtoSource(namespace, content) {
		...
		protoRootMap[namespace] = dcodeIO.protobuf.loadProto(content, namespace);

		return Promise.resolve();
	}

	function addProtoFilePath(namespace, filePath) {
		return dcodeIO.protobuf.load(filePath).then(function (root) {
  ...
  buffer.writeByte(0);*/
		...
		// buffer.writeUint32(type.length);
		// buffer.writeString(type);
		// seq
		var s = seq;
		// buffer.writeUint32(s);
		var code = 0;
  ...
  buffer.writeByte(112);*/
		...
		var message = marshal('game.Message', { type: type, seq: seq, code: code, data: body });
	...
	function unpackHeader(bytes) {
		...
		// var pType = buffer.readUTF8String();
		// var seq = buffer.readUint32();
		// var code = buffer.readUint32();
  	...
  	if ('X-Reply-Error' in meta) {
		...
		var head = unmarshal('game.Message', bodyBytes);
		...
		createClass(Session, [{
			...
			value: function _onMessage(event) {
				...
				var _unpackHeader = unpackHeader(event.data),
					...
					pType = _unpackHeader.type,
					seq = _unpackHeader.seq,
					code = _unpackHeader.code,
					bodyBytes = _unpackHeader.data;

				var body = void 0;
				if (pType.indexOf('Resp') >= 0) {
					//reply
					...
					var error = code;
					if (error != 0) {
						console.log(name+'Resp error error code:',error);
						reject(error);
					} else {
						var repName = name + 'Resp';
						body = unpackBody(bodyBytes, repName);
						console.log(repName+'',body);
						resolve(body);
					}
				} else if (pType.indexOf('Notify') >= 0) {
					//push
					// var router = this.routerMap[uri.url];
					// if (router) {
					body = unpackBody(bodyBytes, 'game.' + pType);
					console.log(pType+'',body);
					this.emit(pType, body);
					// }
				}
			}
		}, {
			...
			value: function request(name, data) {
				...
				return new Promise(function (resolve, reject) {
					if (_this2.socket && _this2.connected) {
						...
						var bytes = pack(reqName, data, ++seq);
						console.log('request',reqName,data);
