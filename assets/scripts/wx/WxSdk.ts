...
export default class WxSdk {
	...
	static checkSession() {
		...
		return new Promise((resolve, reject) => {
			let checkObj = {
				success: (res) => {
					cc.log('checkSession success', res);
					resolve();
				},
				fail: (res) => {
					cc.log('checkSession fail', res);
					reject();
				},
			};
	...
	static createUserInfoButton(successCallback) {
		...
		let top = (fra.height / 2 - (-213) * scaleY) - height / 2;
	...
	static showShareMenu() {
		...
		wx.showShareMenu({
			...
			success: (res) => {
				cc.log('开启分享：' + JSON.stringify(res));
			},
			fail: (res) => {
				cc.log('分享失败：' + JSON.stringify(res));
			}
		});
	}
	 ...
	 * @param query string 查询字符串，从这条转发消息进入后，可通过 wx.getLaunchInfoSync() 或 wx.onShow() 获取启动参数中的 query。必须是 key1=val1&key2=val2 的格式。
	 * */
	static onShareAppMessage(title=null, imageUrl=null, query=null) {
		if (cc.sys.platform != cc.sys.WECHAT_GAME) {
			return;
		}
		let share = DataMgr.instance.serverConfig.share_info;
	...
	static getShareInfo(shareTicket, successCallback, failCallback) {
		...
		cc.log('get shareInfo:', shareTicket);
		wx.getShareInfo({
				...
				WxSdk.getShareInfo(shareTicket, res => {
					...
					if (res) {
						...
						HttpMgr.instance.reqShareGroupInfo(data).then((data) => {
							...
							cc.log("onShareGroupInfo group succful = ", groupID);
							...
							// cc.log("onShareGroupInfo group groupID_count = ", groupID_count, "/MaxCount = ", maxCount);
							...
							cc.error("onShareGroupInfo group error = ", err.message);
							resolve(result);
						});
					} else {
						resolve(result);
